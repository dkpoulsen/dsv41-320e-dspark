...
NAME=dsv41-pp5-320e-rebal
PORT=${PORT:-9006}
MAXLEN=${MAXLEN:-1048576}
GPU_UTIL=${GPU_UTIL:-0.97}
MAX_BATCHED=${MAX_BATCHED:-2048}
MAX_SEQS=${MAX_SEQS:-8}
MODEL_DIR=${MODEL_DIR:-/home/dkp/reap/dsv41-320E}
LOG_DIR=${LOG_DIR:-/home/dkp/logs}
mkdir -p "$LOG_DIR"
LOG=/home/dkp/logs/vllm_v41_320e_rebal_$(date +%Y%m%d_%H%M%S).log
...
docker run -d --name $NAME --gpus all --network host --ipc host \
  ...
  -e VLLM_PP_LAYER_PARTITION=8,9,9,8,6 \
  -e VLLM_CPU_OFFLOAD_GB_PER_RANK=0,0,0,0,0 \
  -e VLLM_ENGINE_READY_TIMEOUT_S=14400 \
  -e HF_HUB_OFFLINE=1 \
  --entrypoint vllm $IMAGE serve /models/dsv41-320E \
